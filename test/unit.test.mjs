...
test.skip("THE PAYLOAD: the invoice payload's totals equal computeTotals over its own items", async (t) => {
...
test.skip("THE SCALES: the same delta is MAJOR in the invoice payload and MINOR in the quote payload, 100x apart", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  const id = await seed(c);
  await approve(c, id);
...
test.skip("a delta with only additions is quote-ready, and both scales still agree", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
...
test.skip("a negative delta is invoiced as negative quantities, and the note says a credit note is the other way", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
...
test.skip("the document names the client, the reference, the lines with reasons, the delta and the value before and after", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  const id = await seed(c);
  await c.call("change_order_status", { change_order: id, status: "sent", date: "2026-03-13" });
...
test.skip("the document shows earlier approved change orders and other pending ones as separate lines", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  const first = await seed(c);
  await approve(c, first);
