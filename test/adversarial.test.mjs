...
test.skip("a sixth open change order is refused on the free tier, and closing one gives the slot back", async (t) => {
  const { c } = open(t);
  await c.init();
  for (let i = 1; i <= 5; i++) {
...
test.skip("the invoice payload refuses anything but an approved change order, naming the status and date", async (t) => {
...
test.skip("with no shared profile the currency falls back to EUR, VAT is zero and the answer says so", async (t) => {
...
test.skip("D-R100: an invoice item description never carries a bare minor figure into a major-unit document", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  const id = await seed(c);
  await approve(c, id);
