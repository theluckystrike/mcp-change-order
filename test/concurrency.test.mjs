...
test.skip("forty change orders raised from two processes are all stored, with forty distinct ids", async (t) => {
...
test.skip("thirty lines added to one change order from two processes are all kept", async (t) => {
  const { box, a, b } = pair(t, { key: proKey() });
  await Promise.all([a.init(), b.init()]);
